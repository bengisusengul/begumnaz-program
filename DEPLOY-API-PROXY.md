...
# Rastgele bir string: begumnaz-2026-may-secret-xxxxx
```
...
```bash
wrangler deploy
# Çıktı: "Published to https://begumnaz-api-proxy.YOUR-USERNAME.workers.dev"
```

URL'i kaydet — birazdan siteye gireceksin.

### 7. Begümnaz Program'da proxy ayarı

1. Site → **Döngü** sekmesi → en alta in
2. **🛡️ API Proxy** kartı → **Proxy URL** alanı
3. Yapıştır: `https://begumnaz-api-proxy.YOUR-USERNAME.workers.dev`
...
2. Network tab'ında istek `begumnaz-api-proxy.workers.dev/v1/messages`'e gitmeli, `api.anthropic.com`'a değil
3. Response geliyorsa proxy çalışıyor

## Maliyet

- Cloudflare Worker free tier: **100K istek/gün**
- Begümnaz kullanımı: ~5-20 istek/gün → free tier'da ölmez
...
3. Worker silmek istersen: `wrangler delete begumnaz-api-proxy`
