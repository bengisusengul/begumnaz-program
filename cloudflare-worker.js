// Begümnaz Program · Anthropic API Proxy Worker (Phase 3.B-D)
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (env.AUTH_TOKEN) {
      const token = request.headers.get('x-begumnaz-token') || '';
    ...
    // anthropic-version 2023-06-01 — Anthropic'in geçerli stable versiyonu (PDF document content type 2023-06-01'de stable)
...
function corsHeaders(request, env) {
  ...
  return {
    ...
    'access-control-allow-headers': 'content-type, x-begumnaz-token, anthropic-version',
